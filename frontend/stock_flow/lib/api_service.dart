...
class ApiService {
  ...
  // --- MÉTODOS STOCK FLOW ---
  Future<Map<String, dynamic>> leerFactura(dynamic imagenPath) async {
    // Pendiente implementar MultipartRequest para enviar la imagen
    // uri: Uri.parse('$_baseUrl/api/vision/leer-factura/')
    return {};
  }

  Future<String> consultarAsistente(String pregunta) async {
    final res = await post('/api/asistente/consultar/', {'pregunta': pregunta});
    return res['respuesta']?.toString() ?? 'Error';
  }

  Future<List<dynamic>> obtenerProductos() async {
    final res = await get('/api/productos/');
    return res as List<dynamic>;
  }

  Future<dynamic> registrarMovimiento(Map<String, dynamic> mov) async {
    return await post('/api/movimientos/', mov);
  }
